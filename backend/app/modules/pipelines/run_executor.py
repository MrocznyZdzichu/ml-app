...
class PipelineRunExecutor:
    """Owns the lifecycle and orchestration of one persisted pipeline run."""

    def __init__(
        self,
        repository_factory: Callable[[], PipelineRepository] = PostgresPipelineRepository,
        users: UserRepository | None = None,
        handler_registry_factory: Callable[[], PipelineStepHandlerRegistry] = PipelineStepHandlerRegistry,
        output_materializer_factory: Callable[[], PipelineOutputMaterializer] = PipelineOutputMaterializer,
        report_materializer_factory: Callable[[], ScoringReportMaterializer] = ScoringReportMaterializer,
    ) -> None:
        self.repository_factory = repository_factory
        self.users = users or PostgresUserRepository()
        self.handler_registry_factory = handler_registry_factory
        self.output_materializer_factory = output_materializer_factory
        self.report_materializer_factory = report_materializer_factory
    ...
    def execute(self, run_id: str) -> dict:
        ...
        if hasattr(run, "created_by") and hasattr(run, "business_case_id"):
            try:
                actor = self.users.get(run.created_by)
        ...
        try:
            ...
            handler_registry = self.handler_registry_factory()
            result = None
            executed_step = None
            ...
            for index, step in enumerate(workflow.steps):
                ...
                if not run.is_dry_run:
                    materialized_manifest, materialized_artifact_ids = self.output_materializer_factory().materialize(
                        run=run,
                        version=version,
                        workflow=workflow,
            ...
            if not run.is_dry_run:
                report_manifests, report_artifact_ids = self.report_materializer_factory().materialize(
                    run=run,
                    version=version,
                    workflow=workflow,
