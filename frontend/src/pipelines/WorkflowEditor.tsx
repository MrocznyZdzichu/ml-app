...
import type { PipelineDefinition } from "./PipelineBuilder";
...
export function normalizeWorkflowDefinition(value: unknown): WorkflowDefinition {
  ...
  if (raw.contract_version !== "2.0") {
    ...
    const empty = legacy.inputs.length === 0 && legacy.steps.length === 0 && legacy.outputs.length === 0;
  ...
  const steps = Array.isArray(raw.steps)
    ? raw.steps.flatMap((item) => {
        ...
        return [{
          ...
          inputs: Array.isArray(step.inputs) ? step.inputs as WorkflowStepDefinition["inputs"] : [],
  ...
  return {
    contract_version: "2.0",
    steps,
    outputs: Array.isArray(raw.outputs) ? raw.outputs as WorkflowDefinition["outputs"] : [],
