...
- Typy artefaktów powinny obsługiwać co najmniej: `dataset`, `data_view`, `feature_transform`, `model_version`, `report`, `metrics`, `deployment`, `prediction_dataset`.
...
### Training Reports i wspólny kontrakt raportów

- Raport treningowy i raport po monitoringu są odrębnymi typami raportów korzystającymi ze wspólnego envelope/kontraktu, a nie jednym uniwersalnym szablonem UI. Aktualnie materializowany jest `training_evaluation_report`; istniejącego workflow monitoringu nie przebudowuj przy pracach nad raportem treningowym.
- Każdy finalny udany krok Training i AutoML tworzy niemutowalny artifact `report` zależny od modelu i metryk. Kandydackie próby AutoML nie tworzą raportów; explainability jest liczona tylko dla finalnego zwycięzcy.
- Metryki i provenance raportu dotyczą pełnego dozwolonego evaluation scope. SHAP i permutation importance korzystają z osobno opisanej, deterministycznej i ograniczonej próbki; nie wolno przedstawiać próbki explainability jako zakresu metryk.
- SHAP działa przez bounded explainery dla wspieranych modeli liniowych i drzewiastych. Dla pozostałych modeli raport zachowuje permutation importance oraz jawny powód braku SHAP.
- Raport przechowuje wybraną recepturę AutoFE, decyzje i stabilność selekcji, parametry modelu, validation/search provenance, warnings i lineage. Nie kopiuje danych wierszowych.
...
- Historyczny pierwszy etap wybierał jedną deterministyczną recepturę. Obecna implementacja wykonuje model-aware joint search bazowych profili, warunkowych receptur numerycznych, target-free generowania cech, fold-local supervised feature selection oraz leakage-safe receptur kategorycznych. Nazwy `Planner v2/v3/v4` pozostają nazwami wersji kontraktów/provenance; UI opisuje te funkcje językiem użytkowym.
...
- Obecne transformacje obejmują medianową imputację liczb, opcjonalne wskaźniki braków, skalowanie, bounded one-hot/frequency, hashing, cross-fitted target mean, ordered target encoding, proste cechy kalendarzowe oraz odrzucanie prawdopodobnych identyfikatorów i niewspieranych typów. Unknown categories korzystają z priory albo deterministycznego hash bucketu; target-aware stan jest fitowany bez użycia targetu kodowanego wiersza.
- Joint search porównuje bazowe profile `scaled_dense`, `tree_unscaled`, `non_negative`, warunkową przestrzeń numeryczną, target-free generowanie log/sqrt/interactions, supervised selection oraz receptury kategoryczne. Supervised selection obsługuje mutual information, F-test, chi-square dla klasyfikacji, L1 i ExtraTrees importance, score-ordered correlation pruning oraz profile compact/balanced/wide. Decyzje selected/dropped i stabilność wyboru między foldami trafiają do fitted state i provenance. Generowanie i selekcja mają jawne limity szerokości oraz memory preflight bez cichego samplingu. Optymalizowana jest pełna ścieżka `receptura FE + selektor + model/hiperparametry` na leakage-safe holdoucie albo fold-local CV.
...
- Zintegrowany AutoFE obsługuje jawne fold-local CV dla tabelarycznej klasyfikacji i regresji: surowe foldy są deterministyczne, a pełna receptura wraz z transformacjami, target-aware encodingiem i supervised selection jest fitowana osobno na train każdego foldu. Wynik zapisuje fold scores, bounded OOF summary, liczebności, seed, wersjonowany recipe contract/hash, fitted-state signatures i stabilność selekcji. Run-scoped cache materializuje FE tylko raz per `recipe + raw fold + seed + input scope`. Brakuje persisted row-level OOF predictions, group/time CV dla AutoFE, learned symbolic interactions, kategorii natywnych, ensemble/stackingu i wielokryterialnego wyboru championa.
- `max_trials` w joint AutoFE jest globalnym limitem dla wszystkich faz, nie gwarantowaną liczbą ukończonych prób. Przed etapami exploration/deepening deterministyczny family round-robin dzieli cap receptur między bazowe/numeric, supervised selection, categorical i combined, aby wcześniejszy generator nie zagłodził późniejszych rodzin. Przy capie wystarczającym na cztery rodziny każda otrzymuje co najmniej jednego kandydata; przydział jest zapisany w provenance. Następnie dwufazowy scheduler daje każdej wykonywalnej recepturze budżet eksploracji, a top-K dzieli pozostałe triale i czas. Starsze definicje bez nowych flag zachowują historyczny ordered prefix i płaski podział.
...
3. **Etap C — bogate, typowane feature generation**
   - Status: częściowo wdrożony. Działa fold-local winsoryzacja, signed-log1p, target-free wybór log1p/sqrt/signed-log i ograniczone interakcje multiply/divide/subtract. Dla kategorii działają bounded one-hot/frequency, hashing, cross-fitted target mean i ordered target encoding z persisted smoothing/unknown contract. Obowiązują jawne width/memory caps, lineage przez Feature Manifest i identyczne zastosowanie zwycięskiej receptury w scoringu. Yeo-Johnson, quantile, binning, learned symbolic interactions, tekst i pozostałe rodziny nie są jeszcze wdrożone w AutoFE search space.
   ...
   - Kategorie: obecny zakres one-hot/frequency/hashing/target mean/ordered jest zakończony; kolejne rozszerzenia wymagają uzasadnionych nowych receptur lub adapterów native categorical.
...
4. **Etap D — feature selection i redukcja wymiaru**
   - Status: feature selection dla obecnego zakresu zakończony. Działają fold-local constant/low-variance, mutual information, F-test, chi-square dla klasyfikacji, L1, ExtraTrees importance i redukcja redundancji korelacyjnej. Metoda/profile width należą do search space, a fitted state zapisuje score, selected/dropped, przyczyny korelacyjne i stabilność między foldami. RFE oraz automatyczne PCA/SVD pozostają przyszłym zakresem.
...
- Fold-local foundation oraz pionowe fragmenty B/C/D obejmujące numeryczne receptury, leakage-safe target encoding, supervised selection, family-fair candidate scheduling, dwufazowy budżet i audytowalny leaderboard są zakończone. Najbliższe brakujące rodziny to typowane Yeo-Johnson/quantile/binning, RFE/PCA, native categorical oraz późniejsze ensemble/stacking. Każde rozszerzenie target-aware musi nadal korzystać z istniejącego fold-local executora.
