...
- Historyczny pierwszy etap wybierał jedną deterministyczną recepturę. Obecna implementacja wykonuje model-aware joint search bazowych profili, warunkowych receptur numerycznych oraz ograniczonych receptur profile-aware Plannera v2; dalszy plan rozszerzenia przestrzeni opisuje sekcja poniżej.
...
- Joint search porównuje bazowe profile `scaled_dense`, `tree_unscaled`, `non_negative` oraz warunkową przestrzeń numeryczną dla nowych draftów: baseline, winsoryzację, signed-log1p, wariant łączony i receptury profile-aware Plannera v2. Planner v2 używa jednego target-free pełnozbiorowego profilu min/max/mean/stddev/skewness/zero share/non-null coverage, aby proponować bezpieczne log1p/sqrt/signed-log oraz ograniczone multiply/divide/subtract interactions. Generowanie ma jawny limit szerokości i dodatkowy memory-aware cap wyliczany z pełnej liczby wierszy; każda decyzja i przyczyna trafia do recipe contract `3.0`. Dla `scaled_dense` niezależnie porównywane są skalowania, profile drzewiaste nie generują redundantnych skalowań, a `non_negative` odrzuca receptury mogące tworzyć wartości ujemne. Constant/low-variance selection działa dla receptur generowanych. Optymalizowana jest pełna ścieżka `receptura FE + selektor + model/hiperparametry` na leakage-safe holdoucie albo fold-local CV.
...
- Zintegrowany AutoFE obsługuje jawne fold-local CV dla tabelarycznej klasyfikacji i regresji: surowe foldy są deterministyczne, a pełna receptura wraz z winsoryzacją i selektorem wariancji jest fitowana osobno na train każdego foldu. Deterministyczne transformacje profile-aware i interakcje są odtwarzane z tej samej zwycięskiej receptury w finalnym reficie i scoringu. Wynik zapisuje fold scores, pełnozakresowe bounded OOF summary, liczebności, seed, etap przypisania, wersjonowany `recipe_contract`, `recipe_hash` i fitted-state signatures. Run-scoped cache materializuje FE tylko raz per `recipe + raw fold + seed + input scope`, a kolejne triale ponownie używają fold Parquetów. Brakuje persisted row-level OOF predictions, group/time CV dla AutoFE, supervised feature selection, target encodingu, learned symbolic interactions, kategorii natywnych, ensemble/stackingu i wielokryterialnego wyboru championa.
...
3. **Etap C — bogate, typowane feature generation**
   - Status: częściowo wdrożony. Działa fold-local winsoryzacja, signed-log1p oraz target-free Planner v2 wybierający log1p/sqrt/signed-log na podstawie profilu rozkładu i generujący ograniczone, zeroodporne interakcje multiply/divide/subtract. Obowiązują jawny width cap i memory-aware cap, recipe contract `3.0`, lineage przez Feature Manifest i identyczne zastosowanie zwycięskiej receptury w scoringu. Yeo-Johnson, quantile, binning, learned symbolic interactions i pozostałe rodziny nie są jeszcze wdrożone w AutoFE search space.
